                            ...
                            <?php foreach ($solicitacoes as $solicitacao) : ?>
                                <?php if (strtolower($solicitacao['status']) != 'pendente') : ?>
                                    <tr>
                                        ...
                                        <td class="text-center"><?= esc($solicitacao['avaliador_username']) ?></td>
                                        <td class="text-center">
                                            <button class="btn btn-info btn-sm visualizar-btn"
                                                data-id="<?= $solicitacao['id'] ?>"
                                                title="Visualizar Solicitação">
...
<script>
    $(document).ready(function() {
        ...
        // Abrir modal ao clicar no botão Visualizar
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        if (data.status) {
                            let statusBadge = '';
                            if (data.status.toLowerCase() === 'aprovada') {
                                statusBadge = '<span class="badge badge-success">Aprovada</span>';
                            } else if (data.status.toLowerCase() === 'rejeitada') {
                                statusBadge = '<span class="badge badge-danger">Rejeitada</span>';
                            } else {
                                statusBadge = '<span class="badge badge-secondary">' + capitalize(data.status) + '</span>';
                            }
                            $('#statusAvaliacao').html(statusBadge);
                        } else {
                            $('#statusAvaliacao').text('-');
                        }

                        $('#avaliador').text(data.avaliador || 'Não avaliado');
                        $('#dataAvaliacao').text(data.data_avaliacao ?
                            new Date(data.data_avaliacao).toLocaleString('pt-BR') : 'Não avaliado');
