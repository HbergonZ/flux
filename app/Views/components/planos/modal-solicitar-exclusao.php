    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="solicitarExclusaoModalLabel">Solicitar Exclusão de Plano</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form id="formSolicitarExclusao" method="post" action="<?= site_url('planos/solicitar-exclusao') ?>">
                <input type="hidden" name="<?= csrf_token() ?>" value="<?= csrf_hash() ?>" />
                <input type="hidden" name="id_plano" id="solicitarExclusaoId">

                <div class="modal-body">
                    <p>Você está solicitando a exclusão do plano: <strong id="planoNameToRequestDelete"></strong></p>
